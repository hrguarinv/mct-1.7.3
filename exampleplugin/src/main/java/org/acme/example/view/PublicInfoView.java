...
/**
 * The <code>PublicInfoViewRole</code> class provides a view exposed in the inspector area, which is
 * the right hand side of a window. This view provides information when the example component is 
 * <em>public</em>. When a <code>AbstractComponent</code> is created, its default visibility is
...
public final class PublicInfoView extends View {
	
	public PublicInfoView(AbstractComponent ac, ViewInfo vi) {
		super(ac,vi);
		...
		view.add(createHeaderRow("Information available for everyone", Color.blue, 15)); //NOI18N
		view.add(new JLabel());
